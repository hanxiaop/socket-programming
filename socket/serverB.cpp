...
ifstream databaseA("database_b.csv");
	...
	if( bind(servSockA, (struct sockaddr*)&sockA_addr, sizeof(sockA_addr)) == 0 ){
		cout << "The ServerB is up and running using UDP on port <22047>. \n";
	} else {
	...
	while(1){
		...
		cout << "The ServerB received input <" << awsSentData << "> \n";
		...
		if (result != "-1"){
			...
			cout << "The Server B has found <1> matches \n";
			...
			cout << "The Server B has found <0> matches \n";
			if (sendto(servSockA, createdBuffer, strlen(createdBuffer), 0, (struct sockaddr      	 					*)&aws_addr,sockSize) < 0) {
       			 perror("sendto");
   			}
			cout << "The Server B finished sending the output to AWS \n" ;
